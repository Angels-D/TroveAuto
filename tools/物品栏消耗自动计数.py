...
*   6. 本工具支持多开, 断线后自动恢复计数
*   7. 本工具使用后, 当前运行目录会生成各玩家的使用日志, 供后续分析
************************** 本工具开源免费, 仅供学习使用 **************************
""")
        ...
        while True:
            for pattern in FIND_USE[0]:
                for address in game.pattern_scan_all(convert_pattern_to_regex(pattern),return_multiple=True):
                    count = read(game,game.read_double,address,FIND_USE[1])
                    if not (count % 1 == 0 and 5 < count <= 9999): continue
                    if count_old and count_old == count: break
                    if count_old and abs(count_old - count) > count_check:
                        count_check = min(9999,count_check*10)
                        continue
                    count_old = count
                    count_check = 10
                    game.write_bytes(address,b'',len(pattern.replace(' ',''))//2)
                    game.log.write(f'{datetime.datetime.now()} {int(count)}\n'.encode())
                    print(f'[{datetime.datetime.now()}] {game.name} {int(count)}\n',end='')
    ...
    while True:
        try:
            for process in process_iter(["pid", "name"]):
                if process.info["name"] != GAME: continue
                Game = pymem.Pymem(process.info["pid"])
                Game.pid = process.info["pid"]
                Game.name = read(Game,Game.read_string,Game.pattern_scan_module(convert_pattern_to_regex(FIND_NAME[0]),GAME),FIND_NAME[1])
                if Game.name in Games: continue
                Game.log = open(os.path.join(SAVE,f'{Game.name} {datetime.datetime.now().strftime("%Y-%m-%d")}.txt'),'ab+', buffering=0)
                Game.work = Thread(target=run,args=(Game,))
                Game.work.daemon = True
                Game.work.start()
                Games[Game.name] = Game
        except Exception as e: pass
        if(Games_Keys != list(Games.keys())):
            Games_Keys = list(Games.keys())
            print(f'-> {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} 玩家列表: {"| ".join(Games_Keys)}')
        time.sleep(5)
