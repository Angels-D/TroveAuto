...
public:
    ...
    typedef std::vector<Address> Offsets;

    Memory() {}
    // 构造函数：通过PID打开进程句柄
    Memory(const DWORD &pid, bool isRoot = true) : hProcess(nullptr), pid(pid), isRoot(isRoot)
    {
        ...
        const Address &end = 0xFFFFFFFF)
    {
        ...
        for (uint32_t i = 0; i < threads; ++i)
        {
            ...
            while (current < chunkEnd && !(once && found.load())) {
                ...
                Address regionStart = reinterpret_cast<Address>(mbi.BaseAddress);
            ...
            default:
                if (c >= 'a' && c <= 'f')
                    c = toupper(c);
                           ...
                           (c >= 'A' && c <= 'F') || c == '?'))
                    throw std::invalid_argument("Invalid character detected: " + std::string(1, c));
                result.push_back(c);
            }
        return result;
    }
    ...
    Address GetModuleAddress(const std::string &moduleName, MODULEENTRY32 me = {}) const
    {
        ...
        do
        {
            ...
            {
                CloseHandle(hSnapshot);
                return reinterpret_cast<Address>(me.modBaseAddr);
            }
        } while (Module32Next(hSnapshot, &me));
    ...
    // 解析多级指针地址
    Address GetAddress(const Address &base, const Offsets &offsets = {}) const
    {
        if (offsets.empty())
            return base;
        Address address = base + offsets[0];
    ...
    std::string ReadMemory(const Address &address, const Offsets &offsets = {}, const size_t maxLen = 255) const
    {
        Address finalAddr = GetAddress(address, offsets);
        if (!finalAddr)
            return {};
...
protected:
    ...
    // 读取字符串
    std::string Read(const Address &address, const size_t &maxLen) const
    {
        std::string str;
        char c;
